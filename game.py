...
# картинки взяты из интернета, просто для набросков кода
...
class Player:  # персонаж, игрок. им можно управлять, он может стрелять в монстров и таскать ресурсы
    def __init__(self):
        self.surf = pg.image.load('images/test_player.png').convert_alpha()
        self.new_surf = pg.transform.scale(self.surf, (self.surf.get_width() / 5, self.surf.get_height() / 5))
